...
with streamlit_analytics.track(unsafe_password="test123"):
    fig = px.box(d2.loc[d2.player.isin(d2.groupby('player').agg({'pick':'size'}).reset_index().sort_values('pick',ascending=False).head(25)['player'])], x="player", y="pick", points="all", hover_data=['team','date','source'], title='Distribution of Draft Position by Player', width=1600)
...
for i,r in df.loc[df['source_date'] == draft].sort_values('pick',ascending=True).reset_index(drop=True):
    st.image(r['team_img'])
